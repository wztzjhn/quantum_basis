...
void pbc(const MKL_INT &Lx, const double &Delta, const MKL_INT &step);
...
//        Hamiltonian_xx += temp3;
    ...
    for (MKL_INT j = 0; j < dim_all; j++) {
        ...
        for (MKL_INT cnt = 0; cnt < intermediate_state.size(); cnt++) {
            ...
            MKL_INT i = binary_find(basis_all.begin(), basis_all.end(), ele_new.first) - basis_all.begin();
            if (i <= j) {
...
//    }
    ...
    double Q = 2.0 * qbasis::pi * 1.0 / static_cast<double>(Lx);
    auto SxQ = Sx_Q(Lx, Q);
    auto SyQ = Sy_Q(Lx, Q);
    auto SzQ = Sz_Q(Lx, Q);
    //SxQ.prt();
    ...
    for (MKL_INT j = 0; j < dim_all; j++) {
        ...
        for (MKL_INT cnt = 0; cnt < intermediate_state.size(); cnt++) {
            ...
            MKL_INT i = binary_find(basis_all.begin(), basis_all.end(), ele.first) - basis_all.begin();
