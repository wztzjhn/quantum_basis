...
//  ----------------------------------------------------------------------------
    ...
    public:
        ...
        /** \brief transform the basis according to the given plan (single given orbital).
         *  sgn = 0 or 1 denoting if extra minus sign generated by transforming fermions */
        mbasis_elem& transform(const std::vector<basis_prop> &props,
                               const std::vector<uint32_t> &plan, int &sgn,
                               const uint32_t &orbital);

        /** \brief transform the basis according to the given plan (all orbitals transform in same way): site1 -> site2 */
        mbasis_elem& transform(const std::vector<basis_prop> &props,
                               const std::vector<uint32_t> &plan, int &sgn);

        /** \brief transform the basis according to the given plan (different orbs transform in different ways): (site1, orb1) -> (site2, orb2)
         *  \param plan: outer vector (orbital), middle vector (site), inner pair(first=site, second=orbital) */
        mbasis_elem& transform(const std::vector<basis_prop> &props,
...
// By default, all diagonal elements are stored, even if they are zero (to be compatible with pardiso)
    ...
    public:
        ...
        // construcotr from lil_mat, and if sym_ == true, use only the upper triangle
...
//  ----------------------------------------------------------------------------
    ...
    */
    ...
    public:
        ...
        /** \brief return a vector containing the positions of each site after rotation (2D-only for now):
         *  x -> x', where (x' - x0) = R (x - x0) */
        std::vector<uint32_t> rotation_plan(const uint32_t &origin, const double &angle) const;

        /** \brief return a vector containing the positions of each site after reflection of a given plane (2D-only for now):
         *  x -> x', where the plane is given by a*x + b*y + c == 0 */
        std::vector<uint32_t> reflection_plan(const std::vector<double> &abc) const;

        /** \brief combine two plans \f$P_\text{total} = P_1 P_2\f$. Length of P = number of orbitals,
         *  and length of P[0] = number of sites. Each pair.first = new site, Each pair.second = new orbital */
        std::vector<std::vector<std::pair<uint32_t,uint32_t>>>
        plan_product(const std::vector<std::vector<std::pair<uint32_t,uint32_t>>> &P1,
