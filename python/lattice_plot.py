...
xmin = np.minimum(np.amin(carts[0][:,0]),np.amin(carts[1][:,0]))
xmax = np.maximum(np.amax(carts[0][:,0]),np.amax(carts[1][:,0]))
ymin = np.minimum(np.amin(carts[0][:,1]),np.amin(carts[1][:,1]))
ymax = np.maximum(np.amax(carts[0][:,1]),np.amax(carts[1][:,1]))

plt.plot(carts[0][:,0],carts[0][:,1],"ro")
